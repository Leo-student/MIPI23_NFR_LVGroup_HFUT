...
from model.network_utils import BaseNetwork
...
class EncoderDecoder(BaseNetwork):
    ...
    def __init__(self):
        ...
        # model += [nn.Upsample(scale_factor=2 , mode="nearest") , nn.Conv2d(n_edf * 16 , n_edf * 8 , kernel_size=3 , stride = 1 , padding =1) , norm(n_edf * 8) , activation]
        # model += [nn.Upsample(scale_factor=2 , mode="nearest") , nn.Conv2d(n_edf * 16 , n_edf * 8 , kernel_size=3 , stride = 1 , padding =1) , norm(n_edf * 8) , activation]
        model += [nn.Upsample(scale_factor=2 , mode="bicubic") , nn.Conv2d(n_edf * 16 , n_edf * 8 , kernel_size=3 , stride = 1 , padding =1) , norm(n_edf * 8) , activation]
        model += [nn.Upsample(scale_factor=2 , mode="bicubic") , nn.Conv2d(n_edf * 8 , n_edf * 4 , kernel_size=3 , stride = 1 , padding =1) , norm(n_edf * 4) , activation]
