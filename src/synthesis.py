...
def add_flare_paper(
    ...
    scene_linear = (scene_linear * random.uniform(0.5, 1.2)).clamp(0, 1)
    ...
    flare_srgb.clamp_(min=0.0, max=1.0)
    scene_srgb.clamp_(min=0.0, max=1.0)
...
def add_mask(combined, flare, gamma=2.2):
    # Avoid zero. Otherwise, the gradient of pow() below will be undefined when
    # gamma < 1.
    combined = combined.clamp(_EPS, 1.0)
    N,_,_,_ = combined.shape
    flare = flare.repeat([1,3,1,1])
    flare = flare.clamp(_EPS, 1.0)
    ...
    # gamma < 1.
    scene_linear = scene_linear.clamp(_EPS, 1.0)
    scene_mask = torch.pow(scene_linear, 1.0 / gamma)
    return scene_mask

# def get_highlight_mask(image, threshold=0.9):
def get_highlight_mask(image, threshold=0.97):
...
def _test():
    scene = utils.load_image(
        r"/home/workspace/MAT/Flickr24K/156.jpg",
        #size=(300, 300),
    )
    flare = utils.load_image(
        ...
        r"/home/workspace/dataset/flare/flare/Flare/img_001197.png",
        #size=(683, 512),
    )
    print(f"Before {flare.size()}, {scene.size()}")
    #Before torch.Size([1, 3, 1440, 1440])
    scene = scene.repeat(4, 1, 1, 1)
    flare = flare.repeat(4, 1, 1, 1)
    #After torch.Size([4, 3, 1440, 1440])
    ...
    #print(f'flare_free.szie = {np.array(combined).shape}')   
    #flare_free = remove_flare(combined, flare, gamma=2.2)

    #torchvision.utils.save_image(flare_free, r'/home/workspace/dataset/combined_flare/flare_free.jpg')
    #print(f'flare_free.szie = {flare_free}')
    torchvision.utils.save_image(
        torch.cat(combined, dim=0), r"/home/workspace/dataset/combined_flare/combined_.jpg", nrow=4
    )
    torchvision.utils.save_image(
        torch.cat(scenes, dim=0), r"/home/workspace/dataset/combined_flare/scene_.jpg", nrow=4
    )
    torchvision.utils.save_image(
        torch.cat(flares, dim=0), r"/home/workspace/dataset/combined_flare/flare_.jpg", nrow=4
    )
    torchvision.utils.save_image(
        torch.cat(flare_segs, dim=0), r"/home/workspace/dataset/combined_flare/flare_segs_.jpg", nrow=4
    )
